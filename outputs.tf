...
output "bastion_connection" {
  description = "Bastion Connection Teste"
  value       = "ssh -i tutorial_kp.pem ubuntu@${aws_eip.tutorial_web_eip[0].public_dns}"
}

// RDS Connection
output "rds_connection" {
  description = "String to RDS Connection - Postgres"
  value       = "ssh -i tutorial_kp.pem -f -N -L 5432:${aws_db_instance.tutorial_database.address}:5432 ubuntu@${aws_eip.tutorial_web_eip[0].public_dns}"
}